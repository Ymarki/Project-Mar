...
namespace _Script.Platform
{
    ...
    {
        /*
        Основной смысл класса "DynamicPlatform" - составить описание объекта "DynamicPlatform"

        1) Скорость с которой объект DynamicPlatform двигается.
        2) Перемещение между верхним и нижним чепоинтами по оси Y.

        А так же проверку, разрешается ли объекту DynamicPlatform разворачиваться.

        Я установил таймер, который запускается сразу после того, как DynamicPlatform развернёться,
        в течении которого, DynamicPlatform не можется развернуться снова.
        */
